...
ACMD(storage)
{
	...
	if (*message && sscanf(message, "%12d", &intval) == 1) {
		if (storage->get_settings(intval) == NULL) {
			safesnprintf(atcmd_output, sizeof(atcmd_output), msg_fd(fd, 51), storage_name);
		...
		clif->message(fd, msg_fd(fd, 52)); // Please specify a storage name (usage: @storage <skill name>).
		return false;
	}
...
ACMD(storeall)
{
	...
	if (*message && sscanf(message, "%12d", &intval) == 1) {
		...
		clif->message(fd, msg_fd(fd, 52)); // Please specify a storage name (usage: @clearstorage <skill name>).
		return false;
	}
